...
def test_01_position_create_flow(logged_in_page):
    assert create_position(logged_in_page, app_state=app_state), "직책 추가 실패"

def test_02_position_update_flow(logged_in_page):
    assert update_position(logged_in_page, app_state=app_state), "직책 수정 실패"

def test_03_position_delete_flow(logged_in_page):
    assert delete_position(logged_in_page), "직책 삭제 실패"