...
from automation.modules.company_tag.update import update_company_tag
from automation.modules.company_tag.delete import delete_company_tag
...
def test_03_update_company_tag_flow(logged_in_page):
    assert update_company_tag(logged_in_page, app_state=app_state), "회사 태그 수정 실패"

def test_04_retrieve_company_tag_flow(logged_in_page):
    assert retrieve_company_tag(logged_in_page, app_state=app_state), "회사 태그 조회 실패"

def test_05_delete_company_tag_flow(logged_in_page):
    assert delete_company_tag(logged_in_page, app_state=app_state), "회사 태그 삭제 실패"