...
# =====================
# 셀렉터 상수 (External Group Delete Page)
# =====================
...
# 메인 플로우 함수
# =====================
def delete_external_group(page: Page, app_state=None) -> bool:
    """외부 그룹 상세 페이지에서 그룹 삭제 절차를 수행"""
    print("\n외부 그룹 삭제 자동화 시작")
    if not click_delete_button(page):
        print("외부 그룹 삭제 자동화 실패 - click_delete_button\n")
        return False
    if not check_delete_boxes(page):
        print("외부 그룹 삭제 자동화 실패 - check_delete_boxes\n")
        return False
    if not confirm_delete(page):
        print("외부 그룹 삭제 자동화 실패 - confirm_delete\n")
        return False
    print("외부 그룹 삭제 자동화 완료\n")
    return True
