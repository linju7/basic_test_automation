...
# =====================
# 셀렉터 상수 (External Group Retrieve Page)
# =====================
BTN_SEARCH = 'button.btn_search'
INPUT_SEARCH = '#group_search_input'
EXTERNAL_GROUP_NAME_CELL = 'strong.ellipsis_element'


def access_external_group_detail(page: Page, group_name: str):
    """외부그룹 검색 후 상세 페이지로 진입"""
    ...
    page.wait_for_timeout(2000)
    # 검색 결과에서 외부그룹명 클릭
    page.wait_for_selector(EXTERNAL_GROUP_NAME_CELL, timeout=10000)
    page.locator(EXTERNAL_GROUP_NAME_CELL).first.click()
...
def extract_page_external_group_info(page):
    """페이지에서 외부그룹 정보를 추출한다."""
    page_info = {}
    
    # 외부그룹명 추출 (두 가지 위치에서 시도)
    ...
    # 메일링리스트는 외부그룹에서는 "사용 안 함"으로 표시됨
    mailing_elem = page.locator('.detail_item:has(i.hd:text-is("메일링리스트")) p')
    if mailing_elem.count() > 0:
        mailing_status = mailing_elem.text_content().strip()
        page_info['mailing_status'] = mailing_status
    
    return page_info


def compare_external_group_info(group_info, page_info):
    ...
    # 메일링리스트 상태 비교 (외부그룹은 항상 "사용 안 함")
    page_mailing_status = page_info.get('mailing_status', '')
    mailing_match = page_mailing_status == "사용 안 함"
    
    comparisons.append({
        'field': '메일링리스트',
        'input': '사용 안 함',
        'page': page_mailing_status or '없음',
        'match': mailing_match
    })
...
def validate_external_group_info(page, app_state=None):
    """외부그룹 정보를 검증하는 함수"""
    if not app_state or not hasattr(app_state, 'external_group_info') or not app_state.external_group_info:
        print("❌ external_group_info가 없습니다.")
        return False
    
    # 페이지에서 정보 추출
    page_info = extract_page_external_group_info(page)
    
    # 정보 비교
    return compare_external_group_info(app_state.external_group_info, page_info)
