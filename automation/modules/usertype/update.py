...
def fill_usertype_update_fields(page, app_state=None):
    from datetime import datetime
    timestamp = datetime.now().strftime("%m%d%H%M")
    usertype_name = f"자동화유형_{timestamp}_수정"
    # 대표명 입력
    ...
    # 다국어 입력
    lang_map = {
        "Korean": f"자동화유형KR_{timestamp}_수정",
        "English": f"자동화유형EN_{timestamp}_수정",
        "Japanese": f"자동화유형JP_{timestamp}_수정",
        "Chinese (TWN)": f"자동화유형TW_{timestamp}_수정",
        "Chinese (CHN)": f"자동화유형CH_{timestamp}_수정",
    }
