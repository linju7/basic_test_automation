...
# =====================
# 셀렉터 상수 (Company Tag Create Page)
# =====================

# 버튼
BTN_ADD_COMPANY_TAG = 'div.task_area button.lw_btn.right_aligned:text-is("회사 태그 추가")'
...
# 레이어
LAYER_CREATE_TAG = 'div.ly_common h3.title:text-is("회사 태그 만들기")'
...
def open_company_tag_page(page):
    ...
    page.wait_for_selector(BTN_ADD_COMPANY_TAG, timeout=5000)

def click_add_company_tag_button(page):
    """회사 태그 추가 버튼 클릭"""
    btn = page.locator(BTN_ADD_COMPANY_TAG)
    if btn.count() > 0:
        btn.first.click()
...
def fill_tag_name(page, app_state=None):
    ...
    tag_name = f"회사태그_자동화_{timestamp}"
...
def create_company_tag(page, app_state=None):
    """회사 태그 추가 플로우를 순차적으로 실행한다. 성공 시 True, 실패 시 False 반환."""
    print("\n회사 태그 추가 자동화 시작")
    open_company_tag_page(page)
    if not click_add_company_tag_button(page):
        print("회사 태그 추가 자동화 실패 - click_add_company_tag_button\n")
        return False
    if not fill_tag_name(page, app_state):
        print("회사 태그 추가 자동화 실패 - fill_tag_name\n")
        return False
    if not click_save_button(page):
        print("회사 태그 추가 자동화 실패 - click_save_button\n")
        return False
    print("회사 태그 추가 자동화 완료\n")
    return True