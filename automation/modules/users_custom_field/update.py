...
# =====================
# 셀렉터 상수 (Users Custom Field Update Page)
# =====================
...
def create_updated_field_info():
    """수정할 구성원 커스텀 필드 데이터를 생성한다."""
    timestamp = datetime.now().strftime("%m%d%H%M")
    
    updated_info = {
        "item_name": f"구성원커스텀_{timestamp}_수정",
        "multilingual_fields": {
            "english": f"구성원커스텀EN_{timestamp}_수정",
            "korean": f"구성원커스텀KR_{timestamp}_수정",
            "japanese": f"구성원커스텀JP_{timestamp}_수정",
            "chinese_simplified": f"구성원커스텀CH_{timestamp}_수정",
            "chinese_traditional": f"구성원커스텀TW_{timestamp}_수정",
        }
    }
...
def save_to_app_state(app_state, updated_info):
    ...
    app_state.users_custom_field_name = updated_info["item_name"]
    app_state.users_custom_field_name_kr = updated_info["multilingual_fields"]["korean"]
    return True
...
# 메인 플로우 함수
# =====================
def update_users_custom_field(page, app_state=None):
    """구성원 커스텀 필드 수정 플로우를 순차적으로 실행한다."""
    print("\n구성원 커스텀 필드 수정 자동화 시작")
    ...
    original_name = app_state.users_custom_field_name
    
    if not save_to_app_state(app_state, updated_info):
        print("구성원 커스텀 필드 수정 자동화 실패 - save_to_app_state\n")
        return False
    
    if not open_custom_field_page(page):
        print("구성원 커스텀 필드 수정 자동화 실패 - open_custom_field_page\n")
        return False
    
    if not find_and_click_modify_button(page, original_name):
        print("구성원 커스텀 필드 수정 자동화 실패 - find_and_click_modify_button\n")
        return False
    
    if not update_item_name(page, updated_info["item_name"]):
        print("구성원 커스텀 필드 수정 자동화 실패 - update_item_name\n")
        return False
    
    if not click_set_global_button(page):
        print("구성원 커스텀 필드 수정 자동화 실패 - click_set_global_button\n")
        return False
    
    if not update_multilingual_fields(page, updated_info["multilingual_fields"]):
        print("구성원 커스텀 필드 수정 자동화 실패 - update_multilingual_fields\n")
        return False
    
    if not click_confirm_global_button(page):
        print("구성원 커스텀 필드 수정 자동화 실패 - click_confirm_global_button\n")
        return False
    
    if not click_save_button(page):
        print("구성원 커스텀 필드 수정 자동화 실패 - click_save_button\n")
        return False
    
    print("구성원 커스텀 필드 수정 자동화 완료\n")
    return True
