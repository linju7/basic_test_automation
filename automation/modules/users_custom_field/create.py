...
# =====================
# 셀렉터 상수 (Users Custom Field Create Page)
# =====================
...
# 항목 추가 레이어
LAYER_ADD_ITEM = 'div.ly_common.ly_page.ly_add_custom_item'
INPUT_ITEM_NAME = 'div.ly_add_custom_item input.lwds_input[maxlength="20"]'
BTN_SET_GLOBAL = 'div.ly_add_custom_item button.btn_set_global'
BTN_ADVANCED_SETTING = 'div.ly_add_custom_item button.btn_advanced_setting'
INPUT_PROPERTY_NAME = 'div.ly_add_custom_item input.lwds_input[maxlength="120"]'
BTN_SAVE = 'div.ly_add_custom_item div.btn_box button.lwds_button_contain:text-is("저장")'
...
def create_custom_field_info():
    """구성원 커스텀 필드 입력 데이터를 생성한다."""
    timestamp = datetime.now().strftime("%m%d%H%M")
    
    custom_field_info = {
        "timestamp": timestamp,
        "item_name": f"구성원커스텀_{timestamp}",
        "property_name": f"user_custom_{timestamp}",
        "multilingual_fields": {
            "english": f"구성원커스텀EN_{timestamp}",
            "korean": f"구성원커스텀KR_{timestamp}",
            "japanese": f"구성원커스텀JP_{timestamp}",
            "chinese_simplified": f"구성원커스텀CH_{timestamp}",
            "chinese_traditional": f"구성원커스텀TW_{timestamp}",
        }
    }
...
def click_advanced_setting_button(page):
    """고급 설정 버튼 클릭"""
    btn = page.locator(BTN_ADVANCED_SETTING)
    if btn.count() > 0:
        btn.first.click()
        page.wait_for_timeout(500)
        return True
    print("[실패] '고급 설정' 버튼을 찾을 수 없음")
    return False
...
def save_to_app_state(app_state, custom_field_info):
    ...
    app_state.users_custom_field_name = custom_field_info["item_name"]
    app_state.users_custom_field_name_kr = custom_field_info["multilingual_fields"]["korean"]
    return True
...
# 메인 플로우 함수
# =====================
def create_users_custom_field(page, app_state=None):
    """구성원 커스텀 필드 추가 플로우를 순차적으로 실행한다."""
    print("\n구성원 커스텀 필드 추가 자동화 시작")
    
    custom_field_info = create_custom_field_info()
    
    if not save_to_app_state(app_state, custom_field_info):
        print("구성원 커스텀 필드 추가 자동화 실패 - save_to_app_state\n")
        return False
    
    if not open_custom_field_page(page):
        print("구성원 커스텀 필드 추가 자동화 실패 - open_custom_field_page\n")
        return False
    
    if not click_add_item_button(page):
        print("구성원 커스텀 필드 추가 자동화 실패 - click_add_item_button\n")
        return False
    
    if not fill_item_name(page, custom_field_info["item_name"]):
        print("구성원 커스텀 필드 추가 자동화 실패 - fill_item_name\n")
        return False
    
    if not click_set_global_button(page):
        print("구성원 커스텀 필드 추가 자동화 실패 - click_set_global_button\n")
        return False
    
    if not fill_multilingual_fields(page, custom_field_info):
        print("구성원 커스텀 필드 추가 자동화 실패 - fill_multilingual_fields\n")
        return False
    
    if not click_confirm_global_button(page):
        print("구성원 커스텀 필드 추가 자동화 실패 - click_confirm_global_button\n")
        return False
    
    if not click_advanced_setting_button(page):
        print("구성원 커스텀 필드 추가 자동화 실패 - click_advanced_setting_button\n")
        return False
    
    if not fill_property_name(page, custom_field_info["property_name"]):
        print("구성원 커스텀 필드 추가 자동화 실패 - fill_property_name\n")
        return False
    
    if not click_save_button(page):
        print("구성원 커스텀 필드 추가 자동화 실패 - click_save_button\n")
        return False
    
    print("구성원 커스텀 필드 추가 자동화 완료\n")
    return True
